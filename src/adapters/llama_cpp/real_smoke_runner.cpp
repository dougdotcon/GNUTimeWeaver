...
int main(int argc, char **argv) {
    ...
    const std::string expected_path=ws+"/expected.bin", pending_path=ws+"/pending.bin";
    int result=0;
    if(mode=="checkpoint") {
        std::string prompt;
        ...
        if(llama_tokenize(vocab,prompt.c_str(),prompt.size(),toks.data(),toks.size(),true,true)<0) result=fail("TOKENIZE_FAILED");
            ...
            if(got!=n || !write_all(state_path,state.data(),state.size()) ||
               !write_tokens(tokens_path,toks) || !write_all(pending_path,&pending,sizeof pending)) result=fail("STATE_WRITE_FAILED");
            else {
        ...
        result=(got==state.size() && pmax+1==(llama_pos)nt && equal)?0:fail("RESTORE_VALIDATION_FAILED");
    } else if(mode=="branch") {
